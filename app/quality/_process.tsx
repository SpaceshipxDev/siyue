...
export function ProcessBoard({
  ...
  function add() {
    if (!jobNo.trim()) return setError('先填工单号')
    if (!reason.trim()) return setError('填一下不良原因')
    setError(null)
    start(async () => {
      try {
        await mutate({
          kind: 'addProcessDefect',
          input: {
            date,
            jobNo: jobNo.trim(),
  ...
  async function patch(id: string, p: Record<string, unknown>) {
    await mutate({ kind: 'updateProcessDefect', defectId: id, patch: p })
    router.refresh()
  }
  ...
  function remove(id: string) {
    start(async () => {
      try {
        await mutate({ kind: 'deleteProcessDefect', defectId: id })
        setArmDelete(null)
        router.refresh()
      } catch (e) {
  ...
  const exportHref = `/quality/export?v=process&m=${year}-${month}${
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-end gap-x-10 gap-y-4">
        <div>
          ...
          <p className="label mt-2.5">{Number(month)}月制程不良</p>
