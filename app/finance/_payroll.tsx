      ...
      <div className="mx-auto max-w-[860px]">
        ...
        <div className="grid grid-cols-1 gap-x-10 gap-y-1 md:grid-cols-2">
          ...
          <div>
            ...
            {/* 房补单独摆在应发之外 —— 它是从综合工资里拆出来的一块, 但不进
                应发 (应发是计税、算社保的那个口径), 到实发那一步再加回去。
                格子里填的是满勤该给的数, 按实际出勤天数折。 */}
            <Edit
              label="房补"
              detail={
                s.workedDays < s.standardDays && s.housingBaseCny > 0
                  ? `不计入应发 · 出勤 ${num(s.workedDays)}/${s.standardDays} 天 → ${formatCny(s.housingAllowanceCny)}`
                  : '不计入应发，实发时加上'
              }
          ...
          <div>
            ...
            <p className="mt-3 text-[11px] text-[var(--color-ink-4)]">
              ...
              综合工资。房补是个例外：它照样从综合工资里拆（福利那一格减掉
              它），但不计入应发，到实发那一步再加回去——应发 + 房补 − 扣款 =
              实发，一分不差。综合工资没过拆分门槛的人只拆三样——基本工资、加
              班费、绩效（比例另设），剩下的全归福利。缺勤扣在右边扣款栏里。
            </p>
          </div>
        </div>
