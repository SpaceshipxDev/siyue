...
export function MachineDevDashboard({ initial }: { initial: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`${withBase('/api/machines')}?raw=1`, { cache: 'no-store' })
    ...
    <article className="overflow-hidden border border-white/10 bg-[#0e1318]">
      ...
      <div className="p-5">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          ...
          )) : <div className="col-span-full border border-amber-300/20 bg-amber-300/[0.05] px-3 py-3 text-xs text-amber-200">等待采集器 4.0 上传接口能力</div>}
        </div>
        ...
        <details className="mt-5 border border-white/10 bg-black/25">
          <summary className="cursor-pointer px-4 py-3 text-xs font-semibold text-white/60">控制器原始只读返回 · 用于云端后续解析</summary>
          <pre className="max-h-[480px] overflow-auto border-t border-white/10 p-4 font-mono text-[11px] leading-5 text-[#b9c9d8]">
            {JSON.stringify(machine.rawTelemetry, null, 2)}
          </pre>
        </details>
      </div>
    </article>
