...
const STAGE_SCOPE_BY_USER_ID: Record<string, StageScope> = {
  ...
  // 打磨喷漆 — 老板 2026-09-14 又收了一道：只剩 打磨 · 喷漆 · 质量。
  //
  // 丝印去掉了 (那一道不是这个账号在做)；质量加进来是因为后道做完当场就自
  // 检，这一批人接着报质量是实情。手工 和 表处 更早就去掉了：手工是别人的
  // 工位，表处是外协出去做的，从这个账号上报会把工时和进度记到错的地方。
  //
  // 单独写死这一行，不跟 FINISHING_STAGE_SCOPE 走 —— 那个常量还管着新开的
  // 打磨账号 (见 fallbackStageScope)，两件事别绑在一起。
  'u-mpdgdq8f-fn7k40': ['打磨', '喷漆', '质量'], // 打磨喷漆
