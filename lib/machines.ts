...
type SnapshotRow = {
  ...
  raw_telemetry: Record<string, unknown> | null
  collector_id: string
...
const MAX_PROGRAM_SOURCE = 2_097_152
const MAX_RAW_TELEMETRY_BYTES = 524_288
...
function parseMachine(input: unknown, index: number): MachineWireSnapshot {
  ...
  return {
    ...
    programSource: nullableRawText(row.programSource, MAX_PROGRAM_SOURCE),
    ...
    rawTelemetry: jsonObject(row.rawTelemetry, `machines[${index}].rawTelemetry`),
...
async function ingestMachineDatabase(payload: MachineIngest): Promise<void> {
  const ingestId = createHash('sha256')
    .update(`${payload.watcherId}\0${payload.observedAt}`)
    .digest('hex')
  const { error: rawIngestError } = await supabase.from('machine_ingest_batches').upsert({
    id: ingestId,
  ...
  if (rawIngestError) throw rawIngestError
  ...
  const rows = payload.machines.map((machine) => {
    ...
    return {
      ...
      raw_telemetry: machine.rawTelemetry,
      collector_id: payload.watcherId,
      collector_version: payload.watcherVersion,
      error: machine.error,
      updated_at: now,
...
export async function getMachineDashboard(options: { includeRaw?: boolean } = {}): Promise<{
  machines: MachineView[]
  events: MachineEventView[]
  serverTime: string
}> {
  if (process.env.MACHINE_STORE_MODE === 'file') {
    return shapeMachineDashboard(await getMachineFileDashboard(), Boolean(options.includeRaw))
  }
  try {
    return await getMachineDatabaseDashboard(Boolean(options.includeRaw))
  } catch (error) {
    if (!isMissingMachineTable(error)) throw error
    return shapeMachineDashboard(await getMachineFileDashboard(), Boolean(options.includeRaw))
  }
...
const MACHINE_DASHBOARD_COLUMNS = [
  'machine_id', 'machine_name', 'ip_address', 'connected', 'state', 'observed_at', 'last_seen_at',
  'job_started_at', 'current_program', 'program_fingerprint', 'program_modified_at', 'program_size_bytes',
  'program_count', 'main_program_count', 'program_number', 'source_part', 'source_part_path', 'controller',
  'cam_programmed_at', 'estimated_duration_seconds', 'operation_count', 'current_operation', 'operations',
  'tool_numbers', 'spindle_rpm', 'feed_mm_min', 'completed_parts', 'total_completed_parts', 'target_parts',
  'current_cycle_seconds', 'current_cutting_seconds', 'controller_boot_cycle_seconds', 'cutting_today_seconds',
  'telemetry_source', 'runtime_observed_at', 'runtime_latency_ms', 'runtime_error', 'discovery_status',
  'discovery_confidence', 'discovered_services', 'execution_state', 'work_signal', 'work_day',
  'worked_today_seconds', 'online_today_seconds', 'current_cycle_started_at', 'ftp_latency_ms',
  'recent_programs', 'manufacturer', 'model', 'driver', 'capabilities', 'discovery_notes',
  'program_source_truncated', 'program_source_sha256', 'program_source_captured_at',
  'collector_id', 'collector_version', 'error', 'updated_at',
].join(',')

async function getMachineDatabaseDashboard(includeRaw: boolean): Promise<{
  machines: MachineView[]
  events: MachineEventView[]
  serverTime: string
}> {
  const [snapshotResult, eventResult] = await Promise.all([
    supabase.from('machine_snapshots').select(includeRaw ? '*' : MACHINE_DASHBOARD_COLUMNS).order('machine_id'),
    supabase
      .from('machine_events')
  ...
  return {
    machines: ((snapshotResult.data ?? []) as unknown as SnapshotRow[]).map(toMachineView),
...
function shapeMachineDashboard<T extends { machines: MachineView[] }>(dashboard: T, includeRaw: boolean): T {
  if (includeRaw) return dashboard
  return {
    ...dashboard,
...
function normalizeFileMachine(machine: MachineView): MachineView {
  return {
    ...
    rawTelemetry: machine.rawTelemetry ?? {},
  }
}
...
function isMissingMachineTable(error: unknown): boolean {
  ...
  // Some installations have the original machine tables but not the newest
  // telemetry columns/constraints yet. Keep ingestion live through the
  // append-only file fallback until the matching migration is applied.
  return candidate.code === '42P01' ||
    candidate.code === '42703' ||
    candidate.code === '23514' ||
    (typeof candidate.message === 'string' && (
      candidate.message.includes('machine_snapshots') ||
      candidate.message.includes('machine_ingest_batches')
    ))
}
...
function toMachineView(row: SnapshotRow): MachineView {
  return {
    ...
    programSource: row.program_source ?? null,
    ...
    rawTelemetry: row.raw_telemetry ?? {},
    error: row.error,
...
function nullableRawText(value: unknown, max: number): string | null {
  if (value == null || value === '') return null
  if (typeof value !== 'string') throw new Error('raw text field is invalid')
  if (value.length > max) throw new Error(`raw text field exceeds ${max} characters`)
  return value
}
...
function jsonObject(value: unknown, label: string): Record<string, unknown> {
  ...
  if (Buffer.byteLength(serialized, 'utf8') > MAX_RAW_TELEMETRY_BYTES) {
    throw new Error(`${label} exceeds ${MAX_RAW_TELEMETRY_BYTES} bytes`)
  }
