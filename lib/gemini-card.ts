...
读用户上传的图纸、3D 截图、报价单、客户备注。识别这一批文件里实际要做的零件，为每个零件单独写一份工艺卡，覆盖九个工段。

工段：工程 编程 操机 手工 打磨 喷漆 丝印 质量 出货

"喷漆" 和 "丝印" 是两道独立工段：喷漆写底/面漆参数、层数、温度、烘烤；丝印写图案、套色顺序、定位、油墨。各自单独成段，不要混写。零件只做其中一道（或都不做）时，把不适用的工段 applies 设为 false。
