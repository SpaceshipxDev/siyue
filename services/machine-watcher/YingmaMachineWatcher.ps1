...
. $discoveryModule
$vendorModule = Join-Path $PSScriptRoot 'YingmaVendorDrivers.ps1'
if (-not (Test-Path -LiteralPath $vendorModule)) { throw "Missing vendor driver module: $vendorModule" }
. $vendorModule
...
function Update-DailyTelemetry {
  ...
  $storedServices = Get-DynamicProperty $State 'discoveredServices'
  $Snapshot.discoveredServices = if ($null -eq $storedServices) { @() } else { @($storedServices) }
  $Snapshot.rawTelemetry = [pscustomobject]@{
    adapter = 'lynuc-modbus'
    transportAvailable = [bool]$runtimeResult.transportAvailable
    exact = [bool]$runtimeResult.exact
    latencyMs = [int]$runtimeResult.latencyMs
    values = $runtimeResult.values
    mapping = $runtime
  ...
  Set-DynamicProperty $State 'currentProgram' ([string]$Snapshot.currentProgram)
  ...
  } elseif ([string](Get-DynamicProperty $State 'programSourceCapturedFor') -ne [string]$Snapshot.currentProgram) {
...
function Get-FtpMachineSnapshot {
  ...
  try {
    ...
    $snapshot.rawTelemetry = [pscustomobject]@{
      adapter = 'ftp'
      selectedProgram = [pscustomobject]@{ name = $latest.Name; sizeBytes = $latest.SizeBytes; modifiedAt = $latest.ModifiedAt }
      directory = @($files | Select-Object -First 256 | ForEach-Object {
        [pscustomobject]@{ name = $_.Name; sizeBytes = $_.SizeBytes; modifiedAt = $_.ModifiedAt }
      })
      analysis = [pscustomobject]@{
        programNumber = $analysis.programNumber
        sourcePart = $analysis.sourcePart
        operationCount = $analysis.operationCount
        toolNumbers = @($analysis.toolNumbers)
        spindleRpm = $analysis.spindleRpm
        feedMmMin = $analysis.feedMmMin
      }
    }
...
function Get-MachineSnapshot {
  param($Machine, $Config, $Previous)
  $driver = Resolve-YingmaCncDriver $Machine
  if ($driver -eq 'focas') { return Get-YingmaFocasSnapshot $Machine $Config $Previous }
  if ($driver -eq 'ezsocket') { return Get-YingmaEzSocketSnapshot $Machine $Config $Previous }
...
function New-SnapshotBase {
  ...
  return [pscustomobject]@{
    ...
    rawTelemetry = [pscustomobject]@{}
    error = $ErrorText
  }
}

function Send-PayloadJson {
  ...
  $result = Invoke-RestMethod `
    ...
    -Body $Json `
    -TimeoutSec ([Math]::Max(20, [int](Get-CncConfigValue $Config 'uploadTimeoutSeconds' 120))) `
    -UseBasicParsing
  if (-not $result.ok) { throw "Server rejected payload: $($result.error)" }
}

function Add-PayloadToSpool {
  param($Payload)
...
function Move-LegacyPendingToSpool {
  if (-not (Test-Path -LiteralPath $PendingPath)) { return }
  try {
    $json = Get-Content -LiteralPath $PendingPath -Raw -Encoding UTF8
    $null = $json | ConvertFrom-Json
    $stamp = [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffffffZ', [Globalization.CultureInfo]::InvariantCulture)
    $path = Join-Path $SpoolDir ("$stamp-legacy.json")
    $utf8NoBom = New-Object Text.UTF8Encoding -ArgumentList $false
    [IO.File]::WriteAllText($path, $json, $utf8NoBom)
    Remove-Item -LiteralPath $PendingPath -Force
    Write-WatcherLog 'Migrated the legacy pending payload into the durable spool'
  } catch {
    Write-WatcherLog "Could not migrate legacy pending payload: $($_.Exception.Message)" 'WARN'
  }
}

function Flush-PayloadSpool {
  param($Config, [int]$Maximum = 20)
  $sent = 0
  $failed = $null
  foreach ($item in @(Get-ChildItem -LiteralPath $SpoolDir -Filter '*.json' -File | Sort-Object Name | Select-Object -First $Maximum)) {
    try {
      $json = Get-Content -LiteralPath $item.FullName -Raw -Encoding UTF8
      Send-PayloadJson $Config $json
  ...
  return [pscustomobject]@{
    sent = $sent
    failed = $failed
    remaining = @(Get-ChildItem -LiteralPath $SpoolDir -Filter '*.json' -File -ErrorAction SilentlyContinue).Count
  }
}
...
if (-not (Get-DynamicProperty $config 'uploadTimeoutSeconds')) { $config | Add-Member -NotePropertyName uploadTimeoutSeconds -NotePropertyValue 120 }
...
if (-not (Get-DynamicProperty $config 'maxProgramReadBytes')) { $config | Add-Member -NotePropertyName maxProgramReadBytes -NotePropertyValue 2097152 }
if (-not (Get-DynamicProperty $config 'maxProgramSourceBytes')) { $config | Add-Member -NotePropertyName maxProgramSourceBytes -NotePropertyValue 2097152 }
...
if ($null -eq $discovery) {
  $discovery = [pscustomobject]@{ enabled = $false; addresses = @(); subnets = @(); ports = @(21, 80, 443, 502, 683, 5000, 8193); maxHosts = 1024 }
...
if ($TestVendor) {
  ...
  foreach ($machine in $config.machines) {
    $driver = Resolve-YingmaCncDriver $machine
    if ($driver -notin @('focas', 'ezsocket')) { continue }
    $result = Get-MachineSnapshot $machine $config $null
    $diagnostics += [pscustomobject]@{
      id = [string]$machine.id; ip = [string]$machine.ip; driver = $driver
      connected = [bool]$result.snapshot.connected; program = $result.snapshot.currentProgram
      executionState = $result.snapshot.executionState; completedParts = $result.snapshot.completedParts
      targetParts = $result.snapshot.targetParts
      currentCycleSeconds = $result.snapshot.currentCycleSeconds
      currentCuttingSeconds = $result.snapshot.currentCuttingSeconds
      controllerBootCycleSeconds = $result.snapshot.controllerBootCycleSeconds
      programSourceBytes = if ($result.snapshot.programSource) { $result.snapshot.programSource.Length } else { 0 }
      error = $result.snapshot.error; runtimeError = $result.snapshot.runtimeError
    }
    if (-not $result.snapshot.connected) {
      $failures += "$($machine.ip) $driver connection/read failed: $($result.snapshot.error)"
    } elseif (-not $result.snapshot.currentProgram) {
      $failures += "$($machine.ip) $driver did not expose a current NC program name"
    } elseif (-not $result.snapshot.programSourceSha256) {
      $failures += "$($machine.ip) $driver exposed the program name but NC source capture failed: $($result.snapshot.runtimeError)"
    } elseif ($null -eq $result.snapshot.completedParts) {
      $failures += "$($machine.ip) $driver did not expose a readable completed-part signal"
    } elseif ($null -eq $result.snapshot.currentCycleSeconds -and
              $null -eq $result.snapshot.currentCuttingSeconds -and
              $null -eq $result.snapshot.controllerBootCycleSeconds) {
      $failures += "$($machine.ip) $driver did not expose any readable duration timer"
    }
  }
  $diagnostics | ConvertTo-Json -Depth 12
...
try {
  ...
  do {
    ...
    $queuedPath = Add-PayloadToSpool $payload
    $flush = Flush-PayloadSpool $config
    if ($flush.failed) {
      Write-WatcherLog "Upload failed; payloads safely queued ($($flush.remaining) pending): $($flush.failed)" 'ERROR'
      if ($Once) { throw "Production endpoint test failed; payload retained at ${queuedPath}: $($flush.failed)" }
    } else {
      Write-WatcherLog "Uploaded $($flush.sent) telemetry batch(es); $($flush.remaining) queued"
    }
